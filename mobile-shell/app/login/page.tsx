...
// Was a Server Action (loginAction) calling straight into Next.js internals
// on the same origin. Server Actions rely on same-origin framing that
// doesn't work from this bundle's capacitor://localhost origin calling the
// Vercel domain, so this now does a plain cross-origin fetch to
// POST /api/auth/login instead -- identical request shape and identical
// server-side logic, just invoked the normal way any API client would.
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/login`, {
        ...
        body: JSON.stringify({ email: emailVal, password: passwordVal, device_id: deviceId }),
      });
